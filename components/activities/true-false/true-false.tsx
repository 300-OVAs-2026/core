...
import { InitialState, OptionRadio, States } from './tyes/types';
import { TrueFalseButton } from './true-false-button';
import { TrueFalseContextProvider } from './true-false-context';
import { Option } from './true-false-option';
...
interface Props {
  ...
  onResult?: ({ result, options }: { result: boolean; options: OptionRadio[] }) => void;
  minSelected?: number;
  addClass?: string;
}
...
export const TrueFalse: React.FC<Props> & SubComponents = ({ children, onResult, minSelected = 1, addClass }: Props) => {
   ...
   */
  const addRadiosValues = ({ id, name, state }: OptionRadio) => {
    updateActivity({
  ...
  return (
    <TrueFalseContextProvider
      ...
      <div className={`${css['wrapper-radios']} ${addClass ?? ''}`}>{children}</div>
    </TrueFalseContextProvider>
  );
};

TrueFalse.Option = Option;
TrueFalse.Button = TrueFalseButton;
