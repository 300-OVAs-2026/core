        ...
        {node.content?.map((listItem: any, idx: number) => (
          <View key={idx} style={styles.listItem}>
            <Text style={styles.listBullet}>
              {node.type === 'bulletList' ? '•' : `${idx + 1}.`}
            </Text>
            <View style={styles.listItemContent}>
              {listItem.content?.map((child: any, childIdx: number) => 
                renderTiptapNode(child, childIdx)
              )}
            </View>
          </View>
        ))}
        ...
        {notes.map((note) => (
          <View key={note.id} style={styles.noteSection} wrap={false}>
            ...
            <View style={styles.contentContainer}>
              {note.content?.content?.map((node: any, idx: number) => 
                renderTiptapNode(node, idx)
              )}
            </View>
          </View>
        ))}
